...
class data_manager(object):
    ...
    def get_train_batch(self):
        ...
        if end >= len(self.train_data): #gone through all data, reshuffle
    ...
    def get_validation_batch(self):
        ...
        '''
        end = (self.t_cursor + 1) * self.val_batch_size
        if end >= len(self.val_data): #gone through all data, reshuffle and reset t_cursor
            np.random.shuffle(self.val_data)
            self.t_cursor = 0
            start, end = 0, self.val_batch_size
        else:
            start = self.t_cursor * self.val_batch_size
        self.t_cursor += 1
    ...
    def load_train_set(self):
        ...
        for elem in self.train_data:
            self.X_train.append(elem['features'])
            self.y_train.append(elem['label'])
    ...
    def load_validation_set(self):
        ...
        for elem in self.val_data:
            self.X_val.append(elem['features'])
            self.y_val.append(elem['label'])
